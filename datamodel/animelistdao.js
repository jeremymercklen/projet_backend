...
module.exports = class AnimeListDAO extends BaseDAO {
    ...
    getAllByUserId(userId) {
        return this.db.query("SELECT * FROM animelist WHERE userid=$1 ORDER BY state, idanime", [userId]);
    }
    ...
    getMostViewedGenres(userId) {
        return this.db.query(`
        SELECT g.name, COUNT(al.id) as count
        FROM genre g
        LEFT JOIN animegenre ag ON g.id = ag.idgenre
        LEFT JOIN animelist al ON ag.idanime = al.idanime AND al.userid = $1
        GROUP BY g.name
